	...
	<div class="flex flex-col gap-6 lg:grid lg:grid-cols-[1fr_18rem] lg:grid-rows-[auto_auto] lg:gap-x-6 lg:gap-y-8 mb-8">
				...
				<div class="flex gap-2 w-full max-w-[36.8rem] lg:max-w-[48rem] h-[20.7rem] sm:h-[23rem] lg:h-[30rem]">
					...
					>
						<img
							...
							style="object-position: {getObjectPosition(data.seeker.Images[0])}"
						/>
					</button>
						...
						<div class="flex-1 flex flex-col gap-2 h-full">
							...
							>
								<img
									...
									class="w-full h-full object-cover hover:opacity-90 transition-opacity"
									style="object-position: {getObjectPosition(data.seeker.Images[1])}"
								/>
							</button>
							{#if data.seeker.Images.length > 2}
								...
								>
									<img
										...
										class="w-full h-full object-cover hover:opacity-90 transition-opacity"
										style="object-position: {getObjectPosition(data.seeker.Images[2])}"
									/>
								</button>
							{/if}
						</div>
		...
		<div class="order-2 lg:order-none space-y-6 lg:col-start-1 lg:row-start-2">
			<div class="flex items-center gap-2 text-muted-foreground">
				...
				<span>{data.seeker.CityDisplay || data.seeker.City}</span>
			</div>
							...
							use:enhance={() => {
								return async ({ result, update }) => {
									if (result.type === 'success' && (result.data as { success?: boolean } | undefined)?.success) {
										trackEvent('generate_lead', { content_type: 'seeker_contact' });
									}
									await update();
								};
							}}
