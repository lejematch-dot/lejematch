...
<div class="max-w-6xl mx-auto px-4 py-6">
	...
	<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4 mb-4 min-w-0">
		<h1 class="text-base sm:text-2xl font-bold text-foreground uppercase tracking-wide truncate min-w-0">{data.listing.Title}</h1>
		{#if data.user}
	...
	<div class="flex flex-col gap-6 lg:grid lg:grid-cols-[1fr_18rem] lg:grid-rows-[auto_auto] lg:gap-x-6 lg:gap-y-8 mb-8">
		...
		<div class="order-2 lg:order-none space-y-6 lg:col-start-1 lg:row-start-2">
			...
			<div class="border-t border-border pt-6">
				...
				<p class="text-muted-foreground whitespace-pre-wrap leading-relaxed {descExpanded ? '' : 'line-clamp-3'}">{data.listing.Description}</p>
				{#if data.listing.Description && data.listing.Description.length > 200}
					<button
						type="button"
